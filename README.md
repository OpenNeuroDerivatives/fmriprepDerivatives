...
3. **`fmriprep --level resample`** — applies the composed transform chain internally to derive denoising **regressors** (motion params, framewise displacement, WM/CSF means, aCompCor components), written to `_desc-confounds_timeseries.tsv`. **Does NOT itself denoise the BOLD** — that's a downstream step.
