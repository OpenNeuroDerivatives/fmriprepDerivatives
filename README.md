...
**Important: `--anat-only` is NOT the same as running raw `recon-all`.** `--anat-only` runs *all of fmriprep's anatomical pipeline*, which includes FreeSurfer's `recon-all` as one step. It also does ANTs N4 bias correction, ANTs skull-strip with population-prior template, ANTs Atropos tissue segmentation, T1w→MNI nonlinear normalization, surface registration to fsLR + fsaverage, and resampling of anatomical outputs into requested template spaces — none of which raw `recon-all` does. Standalone `recon-all` would give you the FreeSurfer subjects-dir; `fmriprep --anat-only` gives you that AND the BIDS-derivatives anatomical scaffold that the later stages reuse.
